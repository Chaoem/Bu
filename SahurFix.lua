-- Sahur_Fixed_SubSection.lua
-- Maru (Sahur) modified: fixed tab/section, AddSection shorthand, section-level AddSection (sub-section labels)
print("Executed - Maru UI Modified (Sahur_Fixed_SubSection)")
...
function Library:AddWindows()
    ...
    function TabFunc:AddTab(cf)
        ...
        -- Store references for this tab's Fe
        if CountTab == 1 then
        ...
        Click_2.Activated:Connect(function()
            ...
            -- safer jump directly to Channel
        ...
        -- Attach channel refs to Fe so sections can reference them safely
        Fe.Channel = Channel
        Fe.ChannelLeft = ChannelLeft
        Fe.ChannelRight = ChannelLeft_2

        function Fe:AddSection(arg1, arg2)
            -- Unified entry for making a Section from a Tab (Fe)
            local andaubuoi, cf
            ...
            cf.Name = cf.Name or "Section"

            -- Decide container (Nigga) based on requested side or default
            local Nigga
            if andaubuoi == "Left" or typeof(andaubuoi) ~= "string" then
                Nigga = Fe.ChannelLeft
            elseif andaubuoi == "Right" then
                Nigga = Fe.ChannelRight
            elseif andaubuoi == "Normal" then
                -- Normal -> append to Channel (full width). Use a dedicated container inside Channel to keep layout stable.
                -- We'll parent sections for "Normal" directly under Channel (after Left/Right), but to avoid overlap, we create a full-width frame area.
                -- Simpler approach: use Channel (parent) and create a Listing inside Channel that spans full width.
                -- For compatibility with existing element logic (which expects a ScrollingFrame / layout), we'll create/use a "ChannelNormal" if not exists.
                local existingNormal = Fe.Channel:FindFirstChild("ChannelNormal")
                if not existingNormal then
                    local NormalFrame = Instance.new("Frame")
                    NormalFrame.Name = "Normal"
                    NormalFrame.Parent = Fe.Channel
                    NormalFrame.BackgroundColor3 = Color3.fromRGB(18,18,18)
                    NormalFrame.BorderSizePixel = 0
                    NormalFrame.Position = UDim2.new(0, 15, 0, 65)
                    NormalFrame.Size = UDim2.new(0.95, 0, 1, -75)

                    local UICorner_Normal = Instance.new("UICorner")
                    UICorner_Normal.Parent = NormalFrame

                    local ChannelNormal = Instance.new("ScrollingFrame")
                    ChannelNormal.Name = "ChannelNormal"
                    ChannelNormal.Parent = NormalFrame
                    ChannelNormal.BackgroundTransparency = 1
                    ChannelNormal.Size = UDim2.new(1, 0, 1, 0)
                    ChannelNormal.ScrollBarThickness = 0

                    local UIListLayout_Normal = Instance.new("UIListLayout")
                    UIListLayout_Normal.Parent = ChannelNormal
                    UIListLayout_Normal.SortOrder = Enum.SortOrder.LayoutOrder
                    UIListLayout_Normal.Padding = UDim.new(0, 12)

                    local UIPadding_Normal = Instance.new("UIPadding")
                    UIPadding_Normal.Parent = ChannelNormal
                    UIPadding_Normal.PaddingTop = UDim.new(0, 8)
                    UIPadding_Normal.PaddingLeft = UDim.new(0, 6)
                    UIPadding_Normal.PaddingRight = UDim.new(0, 6)
                    UIPadding_Normal.PaddingBottom = UDim.new(0, 6)

                    RunService.Stepped:Connect(function()
                        pcall(function()
                            ChannelNormal.CanvasSize = UDim2.new(0, 0, 0, UIListLayout_Normal.AbsoluteContentSize.Y + 20)
                        end)
                    end)
                    Nigga = ChannelNormal
                else
                    Nigga = existingNormal
                end
            else
                -- default to left if unknown string
                Nigga = Fe.ChannelLeft
            end

            -- Create Section frame under chosen Nigga
            ...
            -- The section object returned to caller (Sec) - keep methods to add elements
            local Sec = {}

            -- Provide AddSection on each Sec so you can call RightSide:AddSection({...})
            -- We'll mark it as IsSection and set Sec.Nigga to the Listed frame so elements added afterwards go into this section
            Sec.IsSection = true
            Sec.Nigga = Listed
            Sec.AddSection = Fe.AddSection
            ...
            function Sec:AddDropdown(cffe)
                ...
                function DropFunc:Add(Value)
                    for _, v in next, Value do
                        ...
                        TextButton.Activated:Connect(function()
                            ...
                            pcall(cffe.Callback, Title_5.Text)
                        end)
                    end
                end
                ...
                function DropFunc:Set(acc)
                    for i, v in pairs(Listed_2:GetChildren()) do
                        if v.Name == "Options" and v.Title.Text == acc then
                            for r, a in next, Listed_2:GetChildren() do
                                if a.Name == "Options" then
                                    a.BackgroundTransparency = 1.000
                                    a.Title.TextColor3 = Color3.fromRGB(177, 177, 177)
                            ...
                            pcall(cffe.Callback, v.Title.Text)
                        end
                    end
                end
                ...
                pcall(cffe.Callback, cffe.Default)
                return DropFunc
            end
            ...
            function Sec:AddSlider(cffe)
                ...
                SliderDraggable.BackgroundColor3 = Color3.fromRGB(65, 115, 196)
                ...
                function Slider1:Set(Value)
                    ...
                    pcall(cffe.Callback, Slider1.Value)
                    TweenService:Create(
                        SliderDraggable,
            ...
            function Sec:AddInput(cffe)
                ...
                FreeFire.FocusLost:Connect(function()
                    pcall(cffe.Callback, FreeFire.Text)
                end)
            end

            -- Return the Sec object (section API)
            return Sec
        end
...
print("[Maru UI Modified] Loaded successfully!")
print("Features:")
...
print("✓ AddSection shorthand: AddSection({Name=...})")
print("✓ Section-level AddSection: RightSide:AddSection({Name=...}) creates sub-label")
